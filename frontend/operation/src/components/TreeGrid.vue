                ...
                <tr v-for="(item,index) in initItems" :key="item.id" v-show="show(item)" :class="{'child-tr':item.parent}">
                    <td v-for="(column,snum) in columns" :key="column.key" :style="tdStyle(column)">
                        ...
                        <div v-if="column.type === 'action'">
                            <i-button :type="action.type" size="small" @click="RowClick(item,$event,index,action.text)" v-for='action in (column.actions)' :key="action.text" style="margin-left: 10px;">{{action.text}}</i-button>
                            <!-- <a :type="action.type" size="small" @click="RowClick(item,$event,index,action.text)" v-for='action in (column.actions)' :key="action.text">{{action.text}}</a> -->
                        </div>
                        <label @click="toggle(index,item)" v-if="!column.type">
                            <span v-if='snum==iconRow()'>
                                <i v-html='item.spaceHtml'></i>
                                <i v-if="item.children&&item.children.length>0" class="ivu-icon" :class="{'ivu-icon-ios-plus-outline':!item.expanded,'ivu-icon-ios-minus-outline':item.expanded }"></i>
...
/*自行修改样式*/
.table-bordered>thead>tr>th:nth-of-type(4), .table-bordered>thead>tr>th:nth-of-type(5) {
    width: 200px;
}
</style>